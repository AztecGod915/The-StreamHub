...
function WeeklyScheduleModal({ sportQuery, sportDisplay, onClose }) {
  ...
  const [activeDay, setActiveDay] = useState("today"); // "yesterday"|"today"|"tomorrow"|"all"

  useEffect(() => {
    const sport = getEspnSport(sportQuery);
    if (!sport) { setLoading(false); return; }
    // Always fetch yesterday + today + tomorrow so all tabs work
    ...
    Promise.all(_dates.map(dt =>
      ...
      const _seen = new Set();
      const _allEvts = results.flatMap(d=>(d?.events||[])).filter(e=>{
        if (_seen.has(e.id)) return false; _seen.add(e.id); return true;
      });
      const evts = _allEvts.map(evt=>{
          const comp = evt.competitions?.[0];
          const home = comp?.competitors?.find(c=>c.homeAway==="home")||comp?.competitors?.[0];
          const away = comp?.competitors?.find(c=>c.homeAway==="away")||comp?.competitors?.[1];
          const st = evt.status?.type;
        ...
        TEAM_SPORT_MAP.map(async sp => {
          try {
            ...
            if (sp.fullTournament) {
              ...
              // Fetch past 30 days + next 60 days to show full tournament
              const dateStrs = Array.from({length:90},(_,i)=>{
...
function MoodSearchModal({ onClose, tier, onUpgrade, onResults }) {
  ...
  const search = async () => {
    ...
    try {
      ...
      setResult(parsed);
      // Mood Search is free for all users - no tracking needed
    } catch(e) {
...
export default function StreamHub() {
  ...
  const [planType, setPlanType] = useState(null); // "monthly" | "annual" | "lifetime"
  ...
  const registerPush = async (userId) => {
    try {
      ...
      // Store subscription + favorite teams so server knows what to notify
      const currentFavTeams = (() => {
      ...
      await supabase.from("push_subscriptions").upsert({
        ...
        favorite_teams: currentFavTeams,
        updated_at: new Date().toISOString(),
      }, { onConflict: "user_id" });
    } catch(e) { console.warn("Push registration failed:", e); }
  };

  // Re-sync favorite teams to push_subscriptions whenever they change
  const syncPushFavoriteTeams = async (userId, teams) => {
    try {
      const { data } = await supabase.from("push_subscriptions")
        .select("user_id").eq("user_id", userId).single();
      if (data) {
        await supabase.from("push_subscriptions")
          .update({ favorite_teams: teams, updated_at: new Date().toISOString() })
          .eq("user_id", userId);
      }
    } catch(e) {}
  };
    ...
    setFavoriteTeams(prev => {
      ...
      supabase.auth.getSession().then(({data:{session}})=>{
        if (session?.user) {
          ...
          syncPushFavoriteTeams(session.user.id, updated).catch(()=>{});
  ...
  useEffect(() => {
    ...
    if (params.get("success") === "true") {
      ...
      const upgradeUser = async () => {
        ...
        if (session?.user) {
          const planFromUrl = params.get("plan") || "monthly";
          await supabase.from("profiles").update({ tier:"premium", plan_type: planFromUrl }).eq("id", session.user.id);
          setTier("premium");
  ...
  useEffect(() => {
    ...
    Promise.all(watchlist.slice(0,100).map(async id => {
      try {
        // Try movie first, then TV (covers anime, docs, shows)
        const m = await tmdbFetch(`/movie/${id}?language=en-US`).catch(()=>null);
        if (m?.id) return {...m, media_type:"movie"};
        const t = await tmdbFetch(`/tv/${id}?language=en-US`).catch(()=>null);
        if (t?.id) return {...t, media_type:"tv"};
        return null;
      } catch { return null; }
            ...
            <div style={{display:"flex",gap:8,marginLeft:"auto"}}>
              {tier==="premium"
                ?<button onClick={()=>setShowProfile(true)} style={{
                    ...
                    boxShadow:planType==="lifetime"?"0 0 12px rgba(16,185,129,.5)":planType==="annual"?"0 0 12px rgba(245,158,11,.5)":"0 0 12px rgba(139,92,246,.5)",
                    whiteSpace:"nowrap",letterSpacing:.3,
                  }}>
                    {planType==="lifetime"?"👑 Lifetime Premium":planType==="annual"?"🌟 Annual Premium":"💜 Monthly Premium"}
                  </button>
    ...
    : <span style={{fontSize:11,fontWeight:800,color:"var(--gold)",background:"rgba(245,158,11,.15)",border:"1px solid rgba(245,158,11,.3)",borderRadius:6,padding:"2px 8px"}}>from $4.99/mo →</span>
          ...
          {view==="home"&&!search.trim() ? (
            ...
            <div style={{padding:"0 0 20px"}}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:20,color:"#8B5CF6",marginBottom:6}}>🎬 Documentaries</div>
              <div style={{fontSize:13,color:"var(--muted)",marginBottom:16}}>Top-rated docs across all streaming services</div>
              {(featuredRows.docs||[]).length===0
