...
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:titleSize,color:"var(--gold)",marginBottom:compact?5:7}}>⭐ My Teams</div>

      {/* Team pills — horizontal scroll, no wrap */}
      {allFavTeams.length===0
        ? <div style={{fontSize:9,color:"rgba(245,158,11,.5)",marginBottom:6}}>Tap ⭐ on a sport to follow</div>
        : <div style={{display:"flex",gap:4,overflowX:"auto",scrollbarWidth:"none",marginBottom:compact?5:7,paddingBottom:2}}>
            {allFavTeams.map(({sport,name})=>(
              <button key={sport+name} onClick={()=>{setMode("teams");loadTeam(name);}}
                style={{background:mode==="teams"&&activeTeam===name?"var(--gold)":"rgba(245,158,11,.12)",border:`1px solid ${mode==="teams"&&activeTeam===name?"var(--gold)":"rgba(245,158,11,.3)"}`,borderRadius:99,color:mode==="teams"&&activeTeam===name?"#000":"var(--gold)",padding:"2px 9px",fontSize:compact?9:10,fontWeight:700,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap",transition:"all .15s"}}>
                {name}
              </button>
            ))}
          </div>
      ...
      {/* Game result for selected team */}
      {mode==="teams" && (
        <GameResult result={result} loading={loading} searched={!!activeTeam} query={activeTeam}/>
      )}

      {/* Search Any Team — always visible below */}
      <div style={{marginTop:compact?6:8,paddingTop:compact?5:6,borderTop:"1px solid rgba(245,158,11,.12)"}}>
        <div style={{fontSize:9,color:"rgba(245,158,11,.6)",fontWeight:700,letterSpacing:.5,marginBottom:4}}>🔍 SEARCH ANY TEAM</div>
        <div style={{display:"flex",gap:5}}>
          <input
            value={query}
            onChange={e=>setQuery(e.target.value)}
            onKeyDown={e=>{ if(e.key==="Enter"){ setMode("search"); searchTeam(query); } }}
            onFocus={()=>setMode("search")}
            placeholder="Lakers, Arsenal, Cowboys…"
            style={{flex:1,background:"rgba(0,0,0,.2)",border:"1px solid rgba(245,158,11,.25)",borderRadius:7,color:"var(--text)",padding:compact?"4px 8px":"5px 9px",fontSize:10,outline:"none",minWidth:0}}
          />
          <button onClick={()=>{setMode("search");searchTeam(query);}}
