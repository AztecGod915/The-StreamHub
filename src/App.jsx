...
const BUZZ_CACHE = "sh_sports_buzz_v4";

// ─── SPORTS BUZZ TICKER ───────────────────────────────────────────────────────
// Single-strip marquee: ALL items rendered once in one long div.
// CSS animation scrolls it from 0 → -50% (exactly half the strip width).
// Two identical halves side-by-side means the seam is invisible — no glitch.
// pausedRef stops rAF from updating the manual offset when user pauses.

function SportsNewsTicker() {
  const ALL_ITEMS = [...BUZZ_FALLBACK]; // 18 items: 10 news + 8 odds
  const [items, setItems]   = useState(ALL_ITEMS);
  const [paused, setPaused] = useState(false);
  const pausedRef = useRef(false);

  // ── Load live ESPN headlines and merge with odds ──────────────────────────
  const loadBuzz = async (force=false) => {
    ...
    try {
      ...
      if (live.length >= 4) {
        const ODDS = BUZZ_FALLBACK.filter(i=>i.type==="odds");
        const merged = [...live, ...ODDS];
        setItems(merged);
  ...
  // ── Render one set of pills (rendered twice for the seamless loop) ─────────
  const PillSet = ({suffix}) => items.map((h,i) => {
    ...
    return (
      <span key={`${suffix}-${i}`}
        ...
        style={{
          ...
          cursor:h.url?"pointer":"default", flexShrink:0, marginRight:10,
        }}>
        <span style={{background:p.badge,color:"#fff",fontSize:7,fontWeight:900,
          letterSpacing:.8,borderRadius:99,padding:"1px 5px",flexShrink:0,whiteSpace:"nowrap"}}>
          {p.label}
        </span>
        <span style={{fontSize:11,fontWeight:600,color:"rgba(240,240,250,.9)",whiteSpace:"nowrap"}}>
          {h.emoji} {h.text}
        </span>
      </span>
    );
  ...
  // Duration scales with item count so speed stays consistent regardless of content length
  ...
  return (
    <div style={{
      ...
      {/* Single scrolling strip — two identical halves, animation goes 0 → -50% */}
