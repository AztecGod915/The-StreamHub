...
function PredictionStatsBar({ compact=false }) {
  ...
  if (s.total === 0) return (
    <div style={{
      ...
      borderRadius:16, padding:compact?"10px 10px":"14px 14px",
      display:"flex", flexDirection:"column", gap:compact?6:10,
      cursor:"default", minHeight:compact?90:120,
    }}>
      <div style={{fontSize:compact?20:28,lineHeight:1}}>🔮</div>
      <div>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"#C4B5FD",marginBottom:2}}>Predictions</div>
        {!compact&&<div style={{fontSize:10,color:"rgba(196,181,253,.6)",lineHeight:1.4}}>Tap any game to pick the winner. Build a streak &amp; earn badges.</div>}
        {compact&&<div style={{fontSize:9,color:"rgba(196,181,253,.6)"}}>Tap a game to pick the winner</div>}
      </div>
      <button onClick={sharePredictions} style={{marginTop:"auto",background:"rgba(139,92,246,.2)",border:"1px solid rgba(139,92,246,.4)",borderRadius:8,color:"#C4B5FD",padding:"4px 0",fontSize:9,fontWeight:700,cursor:"pointer",width:"100%"}}>📤 Share</button>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      border:"1.5px solid rgba(139,92,246,.35)",
      borderRadius:16, padding:compact?"10px 10px":"14px 14px",
      display:"flex", flexDirection:"column", gap:0, minHeight:compact?90:120,
    }}>
      {/* Title row */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:compact?6:10}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"#C4B5FD",display:"flex",alignItems:"center",gap:5}}>
          🔮 Predictions {milestone&&<span style={{fontSize:compact?11:14}}>{milestone.icon}</span>}
        </div>
        <button onClick={sharePredictions} style={{background:"rgba(139,92,246,.2)",border:"1px solid rgba(139,92,246,.4)",borderRadius:7,color:"#C4B5FD",padding:"2px 6px",fontSize:9,fontWeight:700,cursor:"pointer"}}>📤</button>
      </div>
      {/* 2x2 stats grid */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:compact?4:6,marginBottom:compact?6:10}}>
        {[
          {val:`${s.streak}🔥`,label:"Streak",color:"#F59E0B"},
          {val:`${s.best}⚡`,label:"Best",color:"#C4B5FD"},
          {val:`${acc}%`,label:"Correct",color:"#10B981"},
          {val:`${s.points}`,label:"Points",color:"#F59E0B"},
        ].map(st=>(
          <div key={st.label} style={{background:"rgba(255,255,255,.05)",borderRadius:compact?7:9,padding:compact?"4px 3px":"6px 4px",textAlign:"center"}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:compact?12:14,color:st.color,lineHeight:1}}>{st.val}</div>
            <div style={{fontSize:7,color:"var(--muted)",fontWeight:700,letterSpacing:.4,marginTop:2}}>{st.label}</div>
          </div>
        ))}
      </div>
      {/* Progress bar */}
      {nextMilestone ? (
        <div style={{marginTop:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
            <div style={{fontSize:9,color:"var(--muted)"}}>Next: {nextMilestone.icon} {nextMilestone.label}</div>
      ...
      ) : <div style={{marginTop:"auto",textAlign:"center",fontSize:10,color:"#F59E0B",fontWeight:700}}>🐐 GOAT status</div>}
    </div>
  );
}
        ...
        <div style={{flex:1,minWidth:0}}>
          {[result.away,result.home].map((t,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:i===0?4:0}}>
              ...
              {(result.isLive||result.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:14,color:t.winner?"var(--gold)":"var(--text)",flexShrink:0}}>{t.score}</span>}
            </div>
          ))}
        </div>
...
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  ...
  const [activeTeam, setActiveTeam]     = useState(allFavTeams[0]?.name||null);
  ...
  useEffect(() => {
    const first = allFavTeams[0]?.name;
    if (first) { setActiveTeam(first); loadTeam(first); }
  ...
  const GameCard = ({r, l}) => {
    ...
    if (!r) return null;
    return (
      <div style={{background:"rgba(0,0,0,.25)",borderRadius:8,padding:"6px 8px",marginTop:6,display:"flex",alignItems:"center",gap:8}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:8,color:"rgba(245,158,11,.5)",fontWeight:700,letterSpacing:.8,marginBottom:3}}>{r.sport.toUpperCase()}</div>
          {[r.away,r.home].map((t,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:4,marginBottom:i===0?2:0}}>
              {t.logo&&<img src={t.logo} alt="" style={{width:12,height:12,objectFit:"contain",flexShrink:0}}/>}
              <span style={{fontSize:10,fontWeight:700,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</span>
              {(r.isLive||r.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:11,color:t.winner?"var(--gold)":"var(--text)",flexShrink:0}}>{t.score}</span>}
            </div>
          ))}
        </div>
        <div style={{flexShrink:0,textAlign:"center",minWidth:52}}>
          {r.isLive
            ? <div style={{background:"rgba(239,68,68,.2)",border:"1px solid rgba(239,68,68,.4)",borderRadius:5,padding:"2px 5px",color:"#ef4444",fontWeight:800,fontSize:8}}>🔴 LIVE</div>
            : r.isOver
              ? <div style={{fontSize:8,color:"var(--muted)",fontWeight:700}}>FINAL</div>
              : <div style={{background:"rgba(245,158,11,.1)",border:"1px solid rgba(245,158,11,.2)",borderRadius:5,padding:"2px 5px"}}>
                  <div style={{fontSize:8,fontWeight:700,color:"var(--gold)",whiteSpace:"nowrap"}}>{r.localDate}</div>
                  <div style={{fontSize:7,color:"var(--muted)"}}>{r.localTime}</div>
                </div>
          }
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"var(--gold)",marginBottom:6}}>⭐ My Teams</div>

      {/* Single team pill — acts as dropdown trigger */}
      {allFavTeams.length===0
        ? <div style={{fontSize:9,color:"rgba(245,158,11,.5)",marginBottom:6}}>Tap ⭐ on a sport to follow</div>
        : <div style={{position:"relative",marginBottom:6}}>
            ...
            {dropOpen && (
              <div style={{
                ...
                background:"var(--surface)",border:"1px solid rgba(245,158,11,.35)",
                ...
                {allFavTeams.map(({sport,name})=>(
                  <button key={sport+name} onClick={()=>loadTeam(name)}
                    style={{
                      ...
                      background:activeTeam===name?"rgba(245,158,11,.15)":"transparent",
                      border:"none",borderBottom:"1px solid rgba(255,255,255,.05)",
                      color:activeTeam===name?"var(--gold)":"var(--text)",
                      padding:"8px 14px",fontSize:12,fontWeight:activeTeam===name?700:400,
                      cursor:"pointer",
                    }}
                    onMouseEnter={e=>e.currentTarget.style.background="rgba(245,158,11,.1)"}
                    onMouseLeave={e=>e.currentTarget.style.background=activeTeam===name?"rgba(245,158,11,.15)":"transparent"}>
      ...
      {/* Search bar */}
      <div style={{marginTop:compact?7:9,paddingTop:compact?5:7,borderTop:"1px solid rgba(245,158,11,.1)"}}>
        <div style={{fontSize:9,color:"rgba(245,158,11,.55)",fontWeight:700,letterSpacing:.5,marginBottom:4}}>🔍 SEARCH ANY TEAM</div>
        <div style={{display:"flex",gap:5}}>
          <input
            ...
            style={{flex:1,background:"rgba(0,0,0,.2)",border:"1px solid rgba(245,158,11,.2)",borderRadius:7,color:"var(--text)",padding:compact?"4px 8px":"5px 8px",fontSize:10,outline:"none",minWidth:0}}
          />
          <button onClick={searchTeam}
            style={{background:"var(--gold)",border:"none",borderRadius:7,color:"#000",padding:compact?"4px 9px":"5px 10px",fontWeight:800,fontSize:10,cursor:"pointer",flexShrink:0}}>
            {searching?"…":"Go"}
          </button>
        </div>
  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div onClick={()=>onSearch("FIFA World Cup 2026")}
        style={{
          background:"linear-gradient(160deg,#071a02 0%,#0a3010 45%,#061a04 100%)",
          border:"2px solid rgba(245,158,11,.55)",
          borderRadius:16, marginBottom:12,
          cursor:"pointer", position:"relative", overflow:"hidden",
          boxShadow:"0 8px 32px rgba(245,158,11,.2), 0 0 60px rgba(16,185,129,.06)",
          transition:"all .2s", minHeight:160,
        }}
        onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(245,158,11,.9)";e.currentTarget.style.transform="scale(1.005)";}}
        onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(245,158,11,.55)";e.currentTarget.style.transform="scale(1)";}}>

        {/* Soccer balls — more of them, spread across whole banner */}
        {[
          {s:38, t:"8%",   l:"42%", d:"7s",  dl:"0s",    o:.42},
          {s:26, t:"55%",  l:"55%", d:"9s",  dl:"1.2s",  o:.35},
          {s:44, t:"5%",   l:"68%", d:"11s", dl:"2.5s",  o:.28},
          {s:20, t:"62%",  l:"75%", d:"8s",  dl:"0.6s",  o:.38},
          {s:32, t:"15%",  l:"82%", d:"10s", dl:"3.5s",  o:.32},
          {s:18, t:"70%",  l:"88%", d:"12s", dl:"1.8s",  o:.28},
          {s:50, t:"-10%", l:"60%", d:"14s", dl:"0.3s",  o:.18},
          {s:22, t:"45%",  l:"93%", d:"9s",  dl:"4s",    o:.3},
        ].map((b,i)=>(
          <div key={i} style={{
        ...
        {/* Scrolling country code strip at bottom */}
        {(()=>{
          const CODES = [
            {code:"US",color:"#3C3B6E"},{code:"MX",color:"#006847"},{code:"CA",color:"#CC0000"},
            {code:"BR",color:"#009C3B"},{code:"AR",color:"#4A90D9"},{code:"FR",color:"#002395"},
            {code:"DE",color:"#444"},{code:"ES",color:"#AA151B"},{code:"PT",color:"#006600"},
            {code:"JP",color:"#BC002D"},{code:"KR",color:"#003478"},{code:"NG",color:"#008751"},
            {code:"MA",color:"#C1272D"},{code:"SA",color:"#006C35"},{code:"CO",color:"#b8930a"},
            {code:"CL",color:"#D52B1E"},{code:"SN",color:"#00853F"},{code:"UY",color:"#4A9FD0"},
            {code:"GH",color:"#006B3F"},{code:"IR",color:"#239F40"},{code:"AU",color:"#00008B"},
            {code:"BE",color:"#000000"},{code:"HR",color:"#CC1414"},{code:"NL",color:"#AE1C28"},
          ];
          const Strip = ({sfx}) => CODES.map((ct,i)=>(
            <span key={sfx+i} style={{
              display:"inline-flex",alignItems:"center",justifyContent:"center",
              width:26,height:17,borderRadius:3,
              background:ct.color,
              color:"#fff",fontSize:7.5,fontWeight:900,letterSpacing:.3,
              marginRight:5,flexShrink:0,
            }}>{ct.code}</span>
          ));
          return (
            <div style={{position:"absolute",bottom:0,left:0,right:0,height:26,overflow:"hidden",pointerEvents:"none",background:"rgba(0,0,0,.25)"}}>
              <div style={{display:"inline-flex",alignItems:"center",padding:"4px 0",whiteSpace:"nowrap",animation:"wcFlags 28s linear infinite"}}>
        ...
        {/* Main content — centred vertically with bottom padding for the strip */}
        <div style={{position:"relative",zIndex:1,padding:"18px 20px 34px",display:"flex",flexDirection:"column",alignItems:"center",gap:10,textAlign:"center"}}>
          ...
          {/* Title — Bebas Neue, large, glowing gold */}
          <div style={{
            fontFamily:"'Bebas Neue', var(--font-head)",
            fontWeight:400,
            fontSize:"clamp(28px,5vw,48px)",
            letterSpacing:3,
            lineHeight:1,
            color:"#F59E0B",
            textShadow:"0 0 30px rgba(245,158,11,.7), 0 0 60px rgba(245,158,11,.35), 0 2px 4px rgba(0,0,0,.8)",
          }}>🏆 FIFA World Cup 2026</div>

          {/* Centred pills row */}
          <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:8}}>
            <div style={{background:"#ef4444",borderRadius:99,padding:"3px 10px",fontSize:9,fontWeight:900,color:"#fff",letterSpacing:.8,animation:"pulse 1.5s infinite",boxShadow:"0 0 10px rgba(239,68,68,.7)"}}>🔴 LIVE SCORES</div>
            <div style={{background:"rgba(245,158,11,.18)",borderRadius:99,padding:"3px 10px",fontSize:9,fontWeight:900,color:"var(--gold)",letterSpacing:.8,border:"1px solid rgba(245,158,11,.4)",boxShadow:"0 0 8px rgba(245,158,11,.3)"}}>ALL TEAMS</div>
          </div>

          {/* Host nations — centred, glowing */}
          <div style={{display:"flex",justifyContent:"center",gap:10}}>
            {[
              {iso:"US",name:"USA",    bg:"linear-gradient(135deg,#3C3B6E,#BF0A30)", glow:"rgba(191,10,48,.6)"},
              {iso:"MX",name:"Mexico", bg:"linear-gradient(135deg,#006847,#CE1126)", glow:"rgba(0,104,71,.6)"},
              {iso:"CA",name:"Canada", bg:"linear-gradient(135deg,#CC0000,#8B0000)",  glow:"rgba(204,0,0,.6)"},
            ].map((h,i)=>(
              <div key={i} style={{
                ...
                boxShadow:`0 0 14px ${h.glow}, 0 0 28px ${h.glow.replace('.6','.25')}`,
              }}>
                <span style={{
                  display:"inline-flex",alignItems:"center",justifyContent:"center",
                  width:26,height:16,borderRadius:3,
                  background:h.bg,
                  color:"#fff",fontSize:8,fontWeight:900,letterSpacing:.4,flexShrink:0,
                  textShadow:"0 0 6px rgba(255,255,255,.8)",
                  boxShadow:`0 0 8px ${h.glow}`,
                }}>{h.iso}</span>
                <span style={{
                  fontSize:11,fontWeight:800,
                  color:"#fff",
                  textShadow:`0 0 10px ${h.glow}, 0 0 20px ${h.glow.replace('.6','.4')}`,
                  letterSpacing:.5,
                }}>{h.name}</span>
              </div>
            ))}
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"var(--surface)",borderRadius:20,width:"100%",maxWidth:600,maxHeight:"85vh",border:"1px solid var(--border)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{overflowY:"auto",padding:"20px 24px",fontSize:13,lineHeight:1.8,color:"rgba(240,240,250,.75)"}}>
          ...
          <p style={{marginBottom:12,background:"rgba(139,92,246,.08)",border:"1px solid rgba(139,92,246,.2)",borderRadius:10,padding:"10px 14px",color:"#C4B5FD",fontSize:12}}>The short version: We collect only what we need to run the service. We don't sell your data. We don't show you ads. You can delete your account and all your data at any time.</p>
          {[
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:0}}>
          ...
          <div style={{
            ...
            <div onClick={()=>setShowMoodSearch(true)}
              style={{
                ...
                borderRadius:14,padding:"12px 14px",
                cursor:"pointer",position:"relative",overflow:"hidden",
                boxShadow:"0 0 24px rgba(139,92,246,.25), 0 4px 16px rgba(139,92,246,.15)",
                display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,
                transition:"all .2s",
              }}
              onTouchStart={e=>e.currentTarget.style.borderColor="rgba(139,92,246,.9)"}
              onTouchEnd={e=>e.currentTarget.style.borderColor="rgba(139,92,246,.55)"}>
              {/* Glow orb */}
              <div style={{position:"absolute",top:-20,right:-20,width:100,height:100,borderRadius:"50%",background:"rgba(139,92,246,.2)",filter:"blur(30px)",pointerEvents:"none"}}/>
              <div style={{display:"flex",alignItems:"center",gap:10,position:"relative"}}>
                <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#8B5CF6,#A855F7)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0,boxShadow:"0 4px 14px rgba(139,92,246,.5)"}}>🎭</div>
                <div>
                  <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:14,background:"linear-gradient(90deg,#c4b5fd,#f0abfc)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Mood Search</div>
                  <div style={{fontSize:10,color:"rgba(196,181,253,.7)"}}>Describe a vibe — AI finds the match</div>
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,position:"relative",flexShrink:0}}>
                <span style={{background:"var(--gold)",color:"#000",fontSize:8,fontWeight:900,padding:"1px 6px",borderRadius:99}}>AI</span>
                <span style={{background:"rgba(139,92,246,.25)",border:"1px solid rgba(139,92,246,.4)",borderRadius:6,padding:"2px 8px",fontSize:9,fontWeight:700,color:"#C4B5FD"}}>Free ✓</span>
              </div>
            </div>
        ...
        {view==="home"&&!safeSearch.trim() ? (
          ...
          <div style={{padding:"12px 14px",overflowY:"auto",flex:1}}>
            {!safeSearch.trim() ? (
              <>
                {/* Predictions + My Teams — side-by-side half-size banners */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                  <FavoriteTeamsBanner favoriteTeams={favoriteTeams} compact={true}/>
                  <PredictionStatsBar compact={true}/>
                </div>
            ...
            <div style={{textAlign:"center"}}>
              ...
              <div onClick={()=>setShowMoodSearch(true)}
                style={{
                  ...
                  cursor:"pointer",position:"relative",overflow:"hidden",
                  boxShadow:"0 0 28px rgba(139,92,246,.25), 0 4px 16px rgba(139,92,246,.15)",
                  display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,
                  transition:"all .2s",pointerEvents:"all",
                }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(139,92,246,.9)";e.currentTarget.style.boxShadow="0 0 36px rgba(139,92,246,.4)";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(139,92,246,.55)";e.currentTarget.style.boxShadow="0 0 28px rgba(139,92,246,.25)";}}>
                <div style={{position:"absolute",top:-20,right:-20,width:120,height:120,borderRadius:"50%",background:"rgba(139,92,246,.18)",filter:"blur(35px)",pointerEvents:"none"}}/>
                <div style={{display:"flex",alignItems:"center",gap:12,position:"relative"}}>
                  <div style={{width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,#8B5CF6,#A855F7)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,boxShadow:"0 4px 16px rgba(139,92,246,.55)"}}>🎭</div>
                  <div>
                    <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:16,background:"linear-gradient(90deg,#c4b5fd,#f0abfc)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Mood Search</div>
                    <div style={{fontSize:11,color:"rgba(196,181,253,.7)"}}>Describe a vibe — AI finds the perfect match</div>
                  </div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,position:"relative",flexShrink:0}}>
                  <span style={{background:"var(--gold)",color:"#000",fontSize:9,fontWeight:900,padding:"2px 8px",borderRadius:99}}>AI</span>
                  <span style={{background:"rgba(139,92,246,.25)",border:"1px solid rgba(139,92,246,.4)",borderRadius:6,padding:"2px 10px",fontSize:10,fontWeight:700,color:"#C4B5FD"}}>Free for all ✓</span>
                </div>
              </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              ...
              <div onClick={()=>setShowMoodSearch(true)}
                style={{
                  ...
                  cursor:"pointer", position:"relative", overflow:"hidden",
                  boxShadow:"0 8px 32px rgba(139,92,246,.3), 0 0 60px rgba(139,92,246,.1)",
                  transition:"all .25s",
                }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(139,92,246,.9)";e.currentTarget.style.boxShadow="0 12px 48px rgba(139,92,246,.55), 0 0 80px rgba(139,92,246,.2)";e.currentTarget.style.transform="translateY(-2px)";}}
                ...
                <div style={{position:"absolute",top:-40,right:-40,width:200,height:200,borderRadius:"50%",background:"rgba(139,92,246,.22)",filter:"blur(55px)",pointerEvents:"none"}}/>
