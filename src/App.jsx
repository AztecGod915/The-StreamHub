        ...
        BUZZ_SOURCES.map(s=>
          fetch(`https://site.api.espn.com/apis/site/v2/sports/${s.path}/news?limit=5`)
            ...
            .then(d=>(d.articles||[]).slice(0,5).map((a,i)=>({
              ...
              emoji: s.emoji,
              type: (a.type||"").toLowerCase().includes("break")||a.premium
                    ? "breaking" : i<=1 ? "trending" : "news",
            })).filter(a=>a.text))
        )
      );
...
<MobileBottomNav view={view} setView={v=>{handleSetView(v);setSearch("");}} watchlist={watchlist} tier={tier} onProfile={()=>user?setShowProfile(true):setShowAuth(true)} />
        ...
        {/* Footer */}
        <div style={{textAlign:"center",padding:"20px 14px 12px",borderTop:"1px solid var(--border)",marginTop:16}}>
