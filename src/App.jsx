...
function SportsNewsTicker() {
  ...
  const handlePauseToggle = () => {
    ...
    // rAF rows check pausedRef each frame — no DOM animation state needed
    if (btnRef.current) btnRef.current.textContent = pausedRef.current ? "▶" : "⏸";
  };

  // rAF-based ticker — moves pixel-by-pixel, repositions silently. No CSS keyframes = no reset flash.
  const RafTickerRow = ({items, reverse}) => {
    const outerRef = useRef(null);
    const innerRef = useRef(null);
    const xRef     = useRef(reverse ? -1 : 0); // stagger rows so they don't move in sync
    ...
    useEffect(() => {
      ...
      // Wait one frame so scrollWidth is measured after paint
      rafRef.current = requestAnimationFrame(() => {
        const halfW = inner.scrollWidth / 2;
        if (reverse && xRef.current === -1) xRef.current = -halfW * 0.4;
    ...
    const renderPills = (keySuffix) => items.map((h, i) => {
      ...
      return (
        <div key={`${keySuffix}-${i}`}
          ...
          style={{
            ...
            cursor:h.url?"pointer":"default",flexShrink:0,marginRight:8,
          }}>
    ...
    return (
      <div ref={outerRef} style={{overflow:"hidden",flex:1,height:"100%",display:"flex",alignItems:"center"}}>
        <div ref={innerRef} style={{display:"inline-flex",alignItems:"center",whiteSpace:"nowrap",willChange:"transform"}}>
          {renderPills("a")}{renderPills("b")}
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{
      ...
      {/* ── Label / pause button ── */}
      <div onClick={handlePauseToggle} style={{
        ...
        gap:4,padding:"6px 4px",textTransform:"uppercase",
      }}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        <div style={{height:35,display:"flex",alignItems:"center",borderBottom:"1px solid rgba(239,68,68,.12)"}}>
          <RafTickerRow items={row1} reverse={false}/>
        </div>
        <div style={{height:35,display:"flex",alignItems:"center"}}>
          <RafTickerRow items={row2} reverse={true}/>
        </div>
      </div>
    </div>
  );
