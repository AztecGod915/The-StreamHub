...
const BUZZ_CACHE = "sh_sports_buzz_v2";

function SportsNewsTicker() {
  const [row1, setRow1] = useState(BUZZ_FALLBACK.slice(0,9));
  const [row2, setRow2] = useState(BUZZ_FALLBACK.slice(9));
  const pausedRef = useRef(false);
  const btnRef    = useRef(null);

  // Betting odds pulled from fallback — always append to fetched results
  ...
  const TickerRow = ({items, reverse, animRef})=>{
    ...
    // Triple the items (not double) so the seam is further away and less likely to be visible
    ...
    return (
      <div style={{overflow:"hidden",flex:1,display:"flex",alignItems:"center",height:"100%",paddingLeft:8}}>
        <div ref={animRef} style={{
          ...
          animation:`${anim} ${spd(items.length)}s linear infinite`,
          animationPlayState:"running",
          whiteSpace:"nowrap",
          // translateX goes to -66.66% (one third) to loop seamlessly across 3 copies
        }}>
          {tripled.map((h,i)=>{
  ...
  return (
    <div
      ...
      <style>{`
        @keyframes buzzForward{from{transform:translateX(0)}to{transform:translateX(-33.333%)}}
        @keyframes buzzReverse{from{transform:translateX(-33.333%)}to{transform:translateX(0)}}
      `}</style>
    </div>
  );
}
    ...
    <div style={{
      ...
      <img
        ...
        style={{
          ...
          display:"block",
          objectFit:"cover",
          maxHeight: mobile ? 180 : tablet ? 240 : 320,
        }}
        onError={e => {
