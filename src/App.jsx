...
const FALLBACK_HEADLINES = [
  {text:"🏆 FIFA World Cup 2026 — Live scores and schedules in the Sports Hub", url:null},
  {text:"⚽ Follow your favorite World Cup team for live score alerts", url:null},
  {text:"🔮 Predict match winners and build your prediction streak", url:null},
  {text:"🎭 Try Mood Search — tell the AI your vibe and find what to watch tonight", url:null},
  {text:"🌙 Watch Tonight — AI picks one perfect thing for you right now", url:null},
  {text:"📊 Track your streaming stats, ratings and watchlist all in one place", url:null},
  {text:"🚨 Leaving Soon alerts — know what's leaving your services before it's gone", url:null},
  {text:"💰 AI Cost Report — find out which streaming services are worth keeping", url:null},
];
...
function SportsNewsTicker() {
  ...
  useEffect(() => {
    // Use today's real game data as ticker content — guaranteed current
    const today = (() => {
      const d = new Date();
    ...
    Promise.allSettled(leagues.map(l =>
      fetch(`https://site.api.espn.com/apis/site/v2/sports/${l.path}/scoreboard?dates=${today}`)
        .then(r=>r.json())
        .then(d=>(d.events||[]).slice(0,4).map(evt=>{
          ...
          const isOver = status?.state==="post";
          const scoreText = (isLive||isOver) && home?.score!==undefined
            ? `${away?.score} - ${home?.score}`
            : comp?.status?.type?.shortDetail||"Scheduled";
          const badge = isLive ? "🔴 LIVE" : isOver ? "FINAL" : "TODAY";
          return {text:`${l.label}: ${awayName} vs ${homeName} · ${badge} ${scoreText}`, url:null};
        }))
    )).then(results=>{
      const items = results
        .flatMap(r=>r.status==="fulfilled"?r.value:[])
        .filter(Boolean);
      if (items.length >= 3) setHeadlines(items);
    }).catch(()=>{});
  }, []);

  const doubled = [...headlines, ...headlines];
  const dur = Math.max(12, headlines.length * 1.5);
          ...
          {doubled.map((h,i)=>(
            <span key={i}
              ...
              style={{
                ...
                color: paused?"rgba(240,240,250,.95)":"rgba(240,240,250,.75)",
                flexShrink:0,
                cursor: h.url?"pointer":"default",
                textDecoration: h.url&&paused?"underline":"none",
                transition:"color .2s",
              }}>
              {h.text}
              <span style={{color:"rgba(255,255,255,.2)",marginLeft:16}}>•</span>
            </span>
          ))}
        </div>
      </div>
      <style>{`@keyframes sportsTicker{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}`}</style>
    </div>
  );
}
