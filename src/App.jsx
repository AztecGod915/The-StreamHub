  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div onClick={()=>onSearch("FIFA World Cup 2026")}
        ...
        {/* Flag marquee — each flag a separate React span so emoji render correctly */}
        {(()=>{
          const FLAGS = ["🇺🇸","🇲🇽","🇨🇦","🇧🇷","🇦🇷","🇫🇷","🇩🇪","🇪🇸","🇵🇹","🇯🇵","🇰🇷","🇳🇬","🇬🇭","🇸🇦","🇲🇦","🇺🇾","🇨🇴","🇨🇱","🇸🇳","🇮🇷"];
          const pills = (sfx) => FLAGS.map((f,i)=>(
            <span key={sfx+i} style={{fontSize:17,marginRight:6,userSelect:"none"}}>{f}</span>
          ));
          return (
            <div style={{position:"absolute",bottom:6,left:0,right:0,height:24,overflow:"hidden",pointerEvents:"none",opacity:.4}}>
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:6}}>
              {[
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(255,255,255,.07)",borderRadius:99,padding:"2px 8px 2px 4px",border:"1px solid rgba(255,255,255,.1)"}}>
                  <span style={{fontSize:15}}>{c.flag}</span>
                  <span style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,.7)"}}>{c.name}</span>
                </div>
              ))}
            </div>
          </div>
