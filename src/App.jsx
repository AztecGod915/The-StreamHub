...
const savePredStats = (s, userId=null) => {
  ...
  if (userId) {
    supabase.from("profiles").update({ pred_stats: JSON.stringify(s) }).eq("id", userId).catch(()=>{});
  }
};
...
export default function StreamHub() {
  ...
  const loadUserData = async (u) => {
    ...
    supabase.from("profiles").update({ login_streak: JSON.stringify(streakData) }).eq("id",u.id).catch(()=>{});
  };
  ...
  const handleSportSearch = (query) => {
    // flushSync forces React to commit BOTH state updates in a single synchronous paint.
    // Without it, view and search update in separate renders causing a blank intermediate frame.
    flushSync(() => {
      setView("sports");
      setSearch(query);
