                ...
                <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(139,92,246,.2)",borderRadius:14,padding:"16px"}}>
                  ...
                  <SubscriptionManagerPanel
                    ...
                    onToggle={async (newSubs)=>{
                      setUserSubs(newSubs);
                      localStorage.setItem("streamhub_subs", JSON.stringify(newSubs));
                      if (user) await supabase.from("profiles").update({subscriptions:JSON.stringify(newSubs)}).eq("id",user.id);
