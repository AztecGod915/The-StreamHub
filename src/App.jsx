...
function PredictionStatsBar({ compact=false }) {
  ...
  if (s.total === 0) return (
    <div style={{
      ...
      <div style={{fontSize:compact?28:40,lineHeight:1,textAlign:"center"}}>🔮</div>
      <div>
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:compact?6:10}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:compact?11:13,color:"#111",display:"flex",alignItems:"center",gap:5}}>
          <span style={{fontSize:compact?22:28,marginRight:4}}>🔮</span>Predictions {milestone&&<span style={{fontSize:compact?11:14}}>{milestone.icon}</span>}
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:compact?4:6,marginBottom:compact?6:10}}>
        {[
          ...
          <div key={st.label} style={{background:"rgba(0,0,0,.1)",border:"1.5px solid rgba(0,0,0,.15)",borderRadius:compact?7:9,padding:compact?"5px 4px":"8px 6px",textAlign:"center"}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:compact?13:16,color:st.color,lineHeight:1}}>{st.val}</div>
            <div style={{fontSize:8,color:"rgba(0,0,0,.55)",fontWeight:700,letterSpacing:.4,marginTop:3}}>{st.label}</div>
          </div>
        ))}
      </div>
      {/* Progress bar */}
      ...
      ) : <div style={{marginTop:"auto",textAlign:"center",fontSize:10,color:"#111",fontWeight:700}}>🐐 GOAT status</div>}
    </div>
  );
}
...
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  ...
  const pad = compact ? "12px 12px" : "16px 16px";
  ...
  const GameCard = ({r, l}) => {
    ...
    if (!r) return null;
    return (
      <div style={{background:"rgba(15,23,42,.85)",border:"1px solid rgba(59,130,246,.3)",borderRadius:10,padding:"10px 12px",marginTop:8,display:"flex",alignItems:"center",gap:8}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:10,color:"#93c5fd",fontWeight:800,letterSpacing:.8,marginBottom:4}}>{r.sport.toUpperCase()}</div>
          {[r.away,r.home].map((t,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:4,marginBottom:i===0?2:0}}>
              {t.logo&&<img src={t.logo} alt="" style={{width:18,height:18,objectFit:"contain",flexShrink:0}}/>}
              <span style={{fontSize:12,fontWeight:700,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#e2e8f0"}}>{t.name}</span>
              {(r.isLive||r.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:13,color:t.winner?"#fbbf24":"#f1f5f9",flexShrink:0}}>{t.score}</span>}
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"#60a5fa",marginBottom:6}}><span style={{fontSize:compact?14:18,marginRight:4}}>🛡️</span>My Teams</div>
