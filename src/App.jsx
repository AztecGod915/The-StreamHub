  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2500,display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"rgba(0,0,0,.6)",backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <button onClick={onClose} style={{background:"rgba(255,255,255,.07)",border:"none",borderRadius:12,color:"rgba(240,240,250,.6)",padding:"10px 28px",fontSize:13,cursor:"pointer",fontWeight:700}}>
          Keep predicting →
        </button>
      </div>
    </div>
    ...
    <div style={{background:"rgba(139,92,246,.07)",border:"1px solid rgba(139,92,246,.2)",borderRadius:16,padding:"14px 16px",marginBottom:16}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          {s.total > 0 && <button onClick={()=>{
            const text = `🔮 I'm on a ${s.streak}-game prediction streak on The StreamHub!

⚡ ${s.streak} correct in a row · 🏆 Best: ${s.best} · 📊 ${s.correct}/${s.total} (${Math.round(s.correct/s.total*100)}% accuracy)${milestone?`

${milestone.icon} ${milestone.label} badge unlocked!`:""}

Think you can beat my streak? Try predictions at thestreamhub.app 👀`;
            navigator.share?navigator.share({title:"My StreamHub Predictions",text,url:"https://thestreamhub.app"}).catch(()=>{}):navigator.clipboard?.writeText(text).then(()=>{});
          }} style={{background:"rgba(139,92,246,.12)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,color:"#C4B5FD",padding:"3px 10px",fontSize:10,fontWeight:700,cursor:"pointer"}}>📤 Share</button>}
        </div>
      </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:12,marginTop:6}}>
                    <a href="/privacy.html" style={{fontSize:11,color:"var(--muted)",textDecoration:"none"}}>Privacy Policy</a>
                    <span style={{color:"rgba(255,255,255,.2)",fontSize:11}}>·</span>
                    <a href="/terms.html" style={{fontSize:11,color:"var(--muted)",textDecoration:"none"}}>Terms of Service</a>
                    <span style={{color:"rgba(255,255,255,.2)",fontSize:11}}>·</span>
