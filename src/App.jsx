  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div onClick={()=>onSearch("FIFA World Cup 2026")}
        style={{
          background:"linear-gradient(135deg,#0d1f05 0%,#0a3010 40%,#0d2008 100%)",
          border:"2px solid rgba(245,158,11,.55)",
          borderRadius:16, padding:"16px 18px", marginBottom:12,
          cursor:"pointer", position:"relative", overflow:"hidden",
          boxShadow:"0 8px 32px rgba(245,158,11,.2), 0 0 40px rgba(16,185,129,.08)",
          transition:"all .2s",
        }}
        onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(245,158,11,.9)";e.currentTarget.style.transform="scale(1.01)";}}
        onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(245,158,11,.55)";e.currentTarget.style.transform="scale(1)";}}>

        {/* Animated soccer balls — boosted opacity so they show on dark green */}
        {[
          {size:32, top:"5%",  left:"62%", dur:"7s",  delay:"0s",   op:.38},
          {size:22, top:"52%", left:"80%", dur:"9s",  delay:"1.5s", op:.3},
          {size:26, top:"15%", left:"88%", dur:"11s", delay:"3s",   op:.25},
          {size:18, top:"68%", left:"70%", dur:"8s",  delay:"0.7s", op:.32},
          {size:38, top:"-8%", left:"74%", dur:"13s", delay:"2s",   op:.2},
        ].map((b,i)=>(
          <div key={i} style={{
            position:"absolute", top:b.top, left:b.left,
            fontSize:b.size, opacity:b.op, pointerEvents:"none",
            animation:`wcFloat ${b.dur} ease-in-out infinite`,
            animationDelay:b.delay, userSelect:"none",
          }}>⚽</div>
        ))}

        {/* Scrolling country code strip — text-based, works everywhere */}
        {(()=>{
          const CODES = [
            {code:"US",color:"#3C3B6E"},
            {code:"MX",color:"#006847"},
            {code:"CA",color:"#FF0000"},
            {code:"BR",color:"#009C3B"},
            {code:"AR",color:"#74ACDF"},
            {code:"FR",color:"#002395"},
            {code:"DE",color:"#000000"},
            {code:"ES",color:"#AA151B"},
            {code:"PT",color:"#006600"},
            {code:"JP",color:"#BC002D"},
            {code:"KR",color:"#003478"},
            {code:"NG",color:"#008751"},
            {code:"MA",color:"#C1272D"},
            {code:"SA",color:"#006C35"},
            {code:"CO",color:"#FCD116"},
            {code:"CL",color:"#D52B1E"},
            {code:"SN",color:"#00853F"},
            {code:"UY",color:"#5EB6E4"},
            {code:"GH",color:"#006B3F"},
            {code:"IR",color:"#239F40"},
          ];
          const Strip = ({sfx}) => CODES.map((c,i)=>(
            <span key={sfx+i} style={{
              display:"inline-flex",alignItems:"center",justifyContent:"center",
              width:24,height:15,borderRadius:3,
              background:c.color,
              color:"#fff",fontSize:7,fontWeight:900,letterSpacing:.3,
              marginRight:6,flexShrink:0,userSelect:"none",
            }}>{c.code}</span>
          ));
          return (
            <div style={{position:"absolute",bottom:8,left:0,right:0,height:18,overflow:"hidden",pointerEvents:"none",opacity:.45}}>
              <div style={{display:"inline-flex",alignItems:"center",whiteSpace:"nowrap",animation:"wcFlags 24s linear infinite"}}>
        ...
        <div style={{position:"relative",zIndex:1,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <div>
            ...
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:20,color:"var(--gold)",lineHeight:1.1,textShadow:"0 0 20px rgba(245,158,11,.4)"}}>🏆 FIFA World Cup 2026</div>
            {/* Host country pills — colored ISO boxes, render everywhere */}
            <div style={{display:"flex",alignItems:"center",gap:6,marginTop:6}}>
              {[
                ...
                {iso:"CA",name:"Canada", bg:"linear-gradient(135deg,#FF0000,#fff2)"},
              ].map((h,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,.08)",borderRadius:99,padding:"3px 10px 3px 5px",border:"1px solid rgba(255,255,255,.12)"}}>
                  <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:22,height:14,borderRadius:3,background:h.bg,color:"#fff",fontSize:7,fontWeight:900,letterSpacing:.3,flexShrink:0}}>{h.iso}</span>
                  <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.85)"}}>{h.name}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{background:"rgba(245,158,11,.12)",border:"1px solid rgba(245,158,11,.35)",borderRadius:10,padding:"8px 14px",textAlign:"center",boxShadow:"0 0 16px rgba(245,158,11,.15)"}}>
