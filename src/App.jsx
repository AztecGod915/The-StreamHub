  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div onClick={()=>onSearch("FIFA World Cup 2026")}
        ...
        {/* Flag marquee — img tags from flagcdn so they render in all browsers */}
        {(()=>{
          const COUNTRIES = ["us","mx","ca","br","ar","fr","de","es","pt","jp","kr","ng","gh","sa","ma","uy","co","cl","sn","ir"];
          const pills = (sfx) => COUNTRIES.map((code,i)=>(
            <img key={sfx+i} src={`https://flagcdn.com/w40/${code}.png`} alt={code}
              style={{width:24,height:16,objectFit:"cover",borderRadius:3,marginRight:8,userSelect:"none",flexShrink:0}}
            />
          ));
          return (
            <div style={{position:"absolute",bottom:8,left:0,right:0,height:20,overflow:"hidden",pointerEvents:"none",opacity:.5}}>
              <div style={{display:"inline-flex",alignItems:"center",whiteSpace:"nowrap",animation:"wcFlags 22s linear infinite"}}>
                {pills("a")}{pills("b")}
              </div>
            </div>
          );
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:6}}>
              {[
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,.07)",borderRadius:99,padding:"3px 10px 3px 6px",border:"1px solid rgba(255,255,255,.12)"}}>
                  <img src={`https://flagcdn.com/w40/${c.code}.png`} alt={c.name} style={{width:20,height:13,objectFit:"cover",borderRadius:2,flexShrink:0}}/>
                  <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.8)"}}>{c.name}</span>
                </div>
              ))}
            </div>
          </div>
