...
const FALLBACK_HEADLINES = [
  {text:"🏆 FIFA World Cup 2026 — Group Stage live across USA, Canada & Mexico", url:"https://www.espn.com/soccer/"},
  {text:"⚽ Live World Cup scores, schedules, and where to watch every game — StreamHub Sports Hub", url:null},
  {text:"🔮 Predict match winners and build your streak in the StreamHub Sports Hub", url:null},
  {text:"🎭 Try Mood Search — tell the AI your vibe and find what to watch tonight", url:null},
  {text:"🌙 Watch Tonight — AI picks one perfect thing for you to watch right now", url:null},
  {text:"⚽ Follow your favorite World Cup team and get notified before every match", url:null},
  {text:"🏆 World Cup 2026 runs through July 19 — catch every game in the Sports Hub", url:"https://www.espn.com/soccer/"},
  {text:"📊 Track your streaming stats, ratings, and watchlist all in one place", url:null},
  {text:"🚨 Get Leaving Soon alerts — know what's leaving your services before it's gone", url:null},
...
function SportsNewsTicker() {
  ...
  useEffect(() => {
    ...
    fetch("https://site.api.espn.com/apis/site/v2/sports/news?limit=20")
      .then(r=>r.json())
      .then(d=>{
        const items = (d.articles||[]).slice(0,15).map(a=>({
          text: a.headline,
          url: a.links?.web?.href || a.links?.app?.href || null,
        })).filter(a=>a.text);
        if (items.length > 0) {
          setHeadlines(items);
                ...
                <SportCategoryGrid onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                  ...
                  <>
                    ...
                    {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                      : search==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                      : search.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                      : <LiveSportsSection sportQuery={search} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult} cols={3}/>}
                  </>
                )}
              </div>
            ) : view==="stats" ? (
