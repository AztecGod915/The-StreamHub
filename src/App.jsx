...
function SportsNewsTicker() {
  const [row1, setRow1] = useState(BUZZ_FALLBACK.slice(0,5));
  const [row2, setRow2] = useState(BUZZ_FALLBACK.slice(5));
  ...
  const loadBuzz = async (force=false) => {
    ...
    try {
      ...
      if (all.length >= 4) {
        ...
        const sorted   = [...breaking,...trending,...news];
        const half     = Math.ceil(sorted.length/2);
        const r1=sorted.slice(0,half), r2=sorted.slice(half);
...
function AppFooter({ onPrivacy, onTerms, mobile=false, tablet=false }) {
  return (
    <div style={{
      background:"linear-gradient(180deg,#0a0818 0%,#06040f 100%)",
      borderTop:"1px solid rgba(245,158,11,.15)",
      padding: mobile ? "20px 16px 96px" : tablet ? "20px 20px 120px" : "28px 40px 36px",
      textAlign:"center",
    }}>
      {/* Banner image */}
      <div style={{
        maxWidth: mobile ? 360 : tablet ? 560 : 860,
        margin:"0 auto",
        borderRadius: mobile ? 14 : 18,
        overflow:"hidden",
        boxShadow:"0 8px 40px rgba(139,92,246,.25), 0 4px 16px rgba(245,158,11,.15)",
        ...
        <img
          ...
          onError={e => {
            // fallback if image not found: render inline banner
            e.target.style.display = "none";
            e.target.parentElement.style.background = "linear-gradient(135deg,#0d0520,#1a0a38)";
            e.target.parentElement.style.padding = mobile ? "24px 16px" : "36px 40px";
            e.target.parentElement.innerHTML = `
              <div style="display:flex;align-items:center;justify-content:center;gap:${mobile?16:32}px;flex-wrap:wrap">
                ...
                <div style="display:flex;gap:${mobile?12:20}px;font-family:var(--font-head);font-weight:800;font-size:${mobile?13:18}px">
      ...
      {/* Links row */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:mobile?10:16,flexWrap:"wrap"}}>
