...
function PredictionStatsBar() {
  ...
  if (s.total === 0) return (
    <div style={{background:"rgba(139,92,246,.07)",border:"1px solid rgba(139,92,246,.2)",borderRadius:12,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
      <span style={{fontSize:20,flexShrink:0}}>🔮</span>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:12}}>Predictions — tap a game to pick the winner</div>
        <div style={{fontSize:10,color:"var(--muted)"}}>Build a streak · earn badges · share your record</div>
      </div>
      <button onClick={sharePredictions} style={{background:"rgba(139,92,246,.15)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,color:"#C4B5FD",padding:"4px 10px",fontSize:10,fontWeight:700,cursor:"pointer",flexShrink:0}}>📤</button>
    </div>
  );

  // Active state — compact two-line layout
  return (
    <div style={{background:"rgba(139,92,246,.07)",border:"1px solid rgba(139,92,246,.2)",borderRadius:12,padding:"10px 12px",marginBottom:12}}>
      {/* Single row: icon + stats + share */}
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:16,flexShrink:0}}>🔮</span>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:12,flex:1}}>
        ...
        {[
          ...
          {val:`${acc}%`,label:"acc",color:"#10B981"},
          {val:`${s.points}`,label:"pts",color:"#F59E0B"},
        ].map(st=>(
          <div key={st.label} style={{textAlign:"center",flexShrink:0}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:13,color:st.color,lineHeight:1}}>{st.val}</div>
            <div style={{fontSize:8,color:"var(--muted)",fontWeight:700,letterSpacing:.4}}>{st.label}</div>
          </div>
        ))}
        <button onClick={sharePredictions} style={{background:"rgba(139,92,246,.12)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,color:"#C4B5FD",padding:"3px 8px",fontSize:10,fontWeight:700,cursor:"pointer",flexShrink:0}}>📤</button>
      </div>
      {/* Progress bar — only if working toward next milestone */}
      {nextMilestone && (
      ...
      {!nextMilestone&&<div style={{textAlign:"center",fontSize:10,color:"#F59E0B",fontWeight:700,marginTop:4}}>🐐 GOAT status — legendary</div>}
    </div>
  );
}
...
function FavoriteTeamsBanner({ favoriteTeams }) {
  ...
  if (allFavTeams.length === 0) return (
    <div style={{background:"rgba(245,158,11,.05)",border:"1px solid rgba(245,158,11,.2)",borderRadius:12,padding:"12px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
      <span style={{fontSize:20}}>⭐</span>
      <div>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:12,color:"var(--gold)"}}>No Favorite Teams Yet</div>
        <div style={{fontSize:10,color:"var(--muted)"}}>Tap the ⭐ on any sport to follow your teams</div>
      </div>
    </div>
  );

  return (
    <div style={{background:"rgba(245,158,11,.05)",border:"1px solid rgba(245,158,11,.2)",borderRadius:12,marginBottom:10,overflow:"hidden"}}>
      <div style={{padding:"10px 14px 8px",borderBottom:"1px solid rgba(245,158,11,.1)"}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:12,color:"var(--gold)",marginBottom:7}}>⭐ My Teams</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {allFavTeams.map(({sport,name})=>(
            <button key={sport+name}
              onClick={()=>loadTeam(name)}
              style={{
                background:activeTeam===name?"var(--gold)":"rgba(245,158,11,.1)",
      ...
      <TeamGameResult result={result} loading={loading} searched={!!activeTeam} query={activeTeam}/>
    </div>
  );
}
