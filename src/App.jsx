...
function CostCalculatorModal({ onClose, userSubs, watchHistory, watchlist, userRatings, tier, onUpgrade }) {
  ...
  const generateAIReport = async () => {
    ...
    try {
      ...
      setAiReport({ summary:"Could not generate report. Check your connection.", keep:[], drop:[], tip:"" });
    }
    setLoading(false);
  };
      ...
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{background:"var(--surface)",borderRadius:22,width:"100%",maxWidth:600,maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column",border:"1px solid rgba(16,185,129,.3)",boxShadow:"0 40px 80px rgba(0,0,0,.8)"}}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"20px 24px 24px"}}>
          ...
          {tab==="overview" && (
            <div>
              {/* Big total */}
              <div style={{background:"linear-gradient(135deg,rgba(16,185,129,.1),rgba(16,185,129,.03))",border:"1px solid rgba(16,185,129,.2)",borderRadius:16,padding:20,textAlign:"center",marginBottom:16}}>
                <div style={{fontSize:11,color:"var(--muted)",marginBottom:6,letterSpacing:1.5}}>MONTHLY STREAMING SPEND</div>
                <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:52,color:"var(--sports)",lineHeight:1}}>${totalMonthly.toFixed(2)}</div>
                <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:10}}>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontSize:16,fontWeight:800,color:"rgba(240,240,250,.8)"}}>${totalAnnual.toFixed(0)}</div>
                    <div style={{fontSize:10,color:"var(--muted)"}}>per year</div>
                  </div>
                  <div style={{width:1,background:"var(--border)"}}/>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontSize:16,fontWeight:800,color:"rgba(240,240,250,.8)"}}>${(totalMonthly/30).toFixed(2)}</div>
                    <div style={{fontSize:10,color:"var(--muted)"}}>per day</div>
                  </div>
                  <div style={{width:1,background:"var(--border)"}}/>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontSize:16,fontWeight:800,color:"rgba(240,240,250,.8)"}}>{totalWatches}</div>
                    <div style={{fontSize:10,color:"var(--muted)"}}>titles watched</div>
                  </div>
                </div>
              </div>

              {/* Per-service cost breakdown with watches */}
              <div style={{marginBottom:16}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:700,fontSize:12,color:"var(--muted)",letterSpacing:1.2,marginBottom:12}}>COST PER SERVICE — THIS MONTH</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {serviceStats.map(s=>{
                    const pct = totalMonthly>0?(s.price/totalMonthly)*100:0;
                    const verdict = s.watches===0&&s.price>0?"⚠️ Unused":s.cpw&&s.cpw<3?"🟢 Great value":s.cpw&&s.cpw<8?"🟡 Average":"🔴 Expensive";
                    return (
                      <div key={s.id} style={{background:"rgba(255,255,255,.03)",border:"1px solid var(--border)",borderRadius:12,padding:"12px 14px"}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            <span style={{background:s.color,borderRadius:7,width:28,height:28,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:900,color:"#fff",flexShrink:0}}>{s.logo}</span>
                            <div>
                              <div style={{fontSize:14,fontWeight:700}}>{s.name}</div>
                              <div style={{fontSize:11,color:"var(--muted)"}}>
                                {s.watches>0?`${s.watches} watch${s.watches!==1?"es":""}  ·  $${s.cpw.toFixed(2)}/watch`:"No watches tracked yet"}
                              </div>
                            </div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontFamily:"var(--font-head)",fontWeight:800,color:"var(--sports)",fontSize:15}}>${s.price.toFixed(2)}</div>
                            <div style={{fontSize:10,color:"var(--muted)"}}>{verdict}</div>
                          </div>
                        </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:700,fontSize:12,color:"var(--muted)",letterSpacing:1.2,marginBottom:4}}>QUICK INSIGHTS</div>
                {mostUsed && <div style={{background:"rgba(16,185,129,.08)",border:"1px solid rgba(16,185,129,.2)",borderRadius:10,padding:12,fontSize:13}}>🏆 <strong>{mostUsed.name}</strong> is your most-used service with {mostUsed.watches} watches this month</div>}
                {bestValue && <div style={{background:"rgba(6,182,212,.08)",border:"1px solid rgba(6,182,212,.2)",borderRadius:10,padding:12,fontSize:13}}>💎 Best value: <strong>{bestValue.name}</strong> at <strong>${bestValue.cpw.toFixed(2)} per watch</strong></div>}
                {worstValue && worstValue.cpw > 10 && <div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",borderRadius:10,padding:12,fontSize:13}}>⚠️ <strong>{worstValue.name}</strong> is costing you <strong>${worstValue.cpw.toFixed(2)} per watch</strong> — worth reconsidering</div>}
                {leastUsed.length>0 && <div style={{background:"rgba(245,197,24,.08)",border:"1px solid rgba(245,197,24,.2)",borderRadius:10,padding:12,fontSize:13}}>💸 You're paying for <strong>{leastUsed.map(s=>s.name).join(", ")}</strong> but haven't watched anything there yet. That's <strong>${leastUsed.reduce((s,sv)=>s+sv.price,0).toFixed(2)}/mo</strong> potentially wasted.</div>}
              </div>

              <button onClick={()=>setTab("ai report")} style={{width:"100%",background:"linear-gradient(135deg,rgba(16,185,129,.2),rgba(6,182,212,.2))",border:"1px solid rgba(16,185,129,.4)",borderRadius:12,color:"var(--sports)",padding:"12px 0",fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,cursor:"pointer"}}>
