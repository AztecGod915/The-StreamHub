...
const BUZZ_FALLBACK = [
  ...
  // Betting odds items
  {text:"ODDS: World Cup Final — Brazil -130 · Argentina +110 · France +140",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: NBA MVP Race — SGA -200 · Wembanyama +180 · Tatum +400",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: UFC — Heavyweight title fight — Aspinall -165 · Jones +140",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: MLB World Series — Dodgers +320 · Yankees +450 · Braves +600",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: EPL Golden Boot — Haaland -150 · Salah +200 · Palmer +350",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: World Cup Top Scorer — Mbappé +250 · Vinicius +300 · Ronaldo +500",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: NFL MVP 2026 season — Lamar Jackson -180 · C.J. Stroud +300",type:"odds",emoji:"📊",url:null},
  {text:"ODDS: WNBA Finals — New York Liberty -140 · Las Vegas Aces +120",type:"odds",emoji:"📊",url:null},
];
...
const BUZZ_CACHE = "sh_sports_buzz_v3";

function SportsNewsTicker() {
  const NEWS_ITEMS = BUZZ_FALLBACK.filter(i=>i.type!=="odds");
  const ODDS_ITEMS = BUZZ_FALLBACK.filter(i=>i.type==="odds");
  // Interleave news + odds so both rows get variety from the start
  const ALL_FALLBACK = [...NEWS_ITEMS, ...ODDS_ITEMS];
  const [row1, setRow1] = useState(ALL_FALLBACK.slice(0, Math.ceil(ALL_FALLBACK.length/2)));
  const [row2, setRow2] = useState(ALL_FALLBACK.slice(Math.ceil(ALL_FALLBACK.length/2)));
  ...
  const loadBuzz = async (force=false) => {
    if (!force) {
      try {
        ...
        if (c.row1?.length && Date.now()-c.ts < 24*60*60*1000) {
          setRow1(c.row1); setRow2(c.row2||[]); return;
        }
      } catch {}
    }
    try {
      const fetched = await Promise.allSettled(
        BUZZ_SOURCES.map(s=>
          fetch(`https://site.api.espn.com/apis/site/v2/sports/${s.path}/news?limit=5`)
            .then(r=>r.json())
            .then(d=>(d.articles||[]).slice(0,5).map((a,i)=>({
              text: a.headline||"",
              url:  a.links?.web?.href||null,
              emoji: s.emoji,
              type: i===0 ? "breaking" : i<=2 ? "trending" : "news",
            })).filter(a=>a.text))
        )
      );
      const all = fetched.filter(r=>r.status==="fulfilled").flatMap(r=>r.value);
      if (all.length >= 4) {
        const breaking = all.filter(a=>a.type==="breaking");
        const trending = all.filter(a=>a.type==="trending");
        const news     = all.filter(a=>a.type==="news");
        // Always include odds items + enough news to prevent loop-reset glitch
        const sorted   = [...breaking,...trending,...news,...ODDS_ITEMS];
        // Ensure minimum 12 items total so each row has ≥6 — prevents short-loop glitch
        const padded   = sorted.length < 12
          ? [...sorted, ...BUZZ_FALLBACK.filter(f=>f.type!=="odds").slice(0, 12-sorted.length)]
          : sorted;
        const half     = Math.ceil(padded.length/2);
        const r1=padded.slice(0,half), r2=padded.slice(half);
        setRow1(r1); setRow2(r2);
        localStorage.setItem(BUZZ_CACHE, JSON.stringify({row1:r1,row2:r2,ts:Date.now()}));
      }
    } catch {}
  };
  ...
  // Speed based on item count — more items = slower (avoids too-fast loop on short lists)
  const spd = n => Math.max(55, n * 6.5);

  // Refs for direct DOM pause — no re-render, freezes at exact position
  const anim1Ref = useRef(null);
  const anim2Ref = useRef(null);

  const handlePauseToggle = () => {
    pausedRef.current = !pausedRef.current;
    const state = pausedRef.current ? "paused" : "running";
    if (anim1Ref.current) anim1Ref.current.style.animationPlayState = state;
    if (anim2Ref.current) anim2Ref.current.style.animationPlayState = state;
    if (btnRef.current)   btnRef.current.textContent = pausedRef.current ? "▶" : "⏸";
  };

  const TickerRow = ({items, reverse, animRef})=>{
    if(!items.length) return null;
    // Need enough items so one pass takes >30s — pad if short
    let padded = [...items];
    while (padded.length < 16) padded = [...padded, ...items];
    const doubled=[...padded,...padded];
    const anim=reverse?"buzzReverse":"buzzForward";
    return (
      <div style={{overflow:"hidden",flex:1,display:"flex",alignItems:"center",height:"100%",paddingLeft:8}}>
        <div ref={animRef} style={{
          display:"flex",gap:8,alignItems:"center",
          animation:`${anim} ${Math.max(60, padded.length * 7)}s linear infinite`,
          animationPlayState:"running",
          whiteSpace:"nowrap",
          willChange:"transform",
        }}>
          {doubled.map((h,i)=>{
            const p=PILL_STYLE[h.type]||PILL_STYLE.news;
            return (
              <div key={i} onClick={()=>h.url&&window.open(h.url,"_blank","noopener")}
                style={{
                  display:"inline-flex",alignItems:"center",gap:5,
                  background:p.bg,border:`1px solid ${p.border}`,
                  borderRadius:99,padding:"2px 10px 2px 4px",
                  cursor:h.url?"pointer":"default",flexShrink:0,
                }}>
                <span style={{background:p.badge,color:"#fff",fontSize:7,fontWeight:900,letterSpacing:.8,borderRadius:99,padding:"1px 5px",flexShrink:0}}>{p.label}</span>
                <span style={{fontSize:11,fontWeight:600,color:"rgba(240,240,250,.9)"}}>{h.emoji} {h.text}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div
      style={{
        ...
        boxShadow:"0 0 20px rgba(239,68,68,.3), 0 0 40px rgba(239,68,68,.12), inset 0 0 20px rgba(239,68,68,.03)",
        display:"flex",
      }}>

      {/* ── Single merged label cell spanning both rows ── */}
      <div
        ...
        <span style={{fontSize:13,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(239,68,68,.9)"}}>⚡</span>
        <span style={{fontSize:13,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(239,68,68,.9)"}}>SPORTS</span>
        <span style={{fontSize:13,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(239,68,68,.9)"}}>BUZZ</span>
        <span ref={btnRef} style={{fontSize:18,lineHeight:1,marginTop:2,textShadow:"0 0 10px rgba(239,68,68,1)"}}>⏸</span>
      </div>

      {/* ── Two ticker rows stacked on the right ── */}
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        <div style={{height:35,display:"flex",alignItems:"center",borderBottom:"1px solid rgba(239,68,68,.12)"}}>
          <TickerRow items={row1} reverse={false} animRef={anim1Ref}/>
        </div>
        <div style={{height:35,display:"flex",alignItems:"center"}}>
          <TickerRow items={row2} reverse={true} animRef={anim2Ref}/>
        </div>
      </div>

      <style>{`
        @keyframes buzzForward{from{transform:translateX(0)}to{transform:translateX(-50%)}}
        @keyframes buzzReverse{from{transform:translateX(-50%)}to{transform:translateX(0)}}
      `}</style>
    </div>
  );
}
  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[{label:"Yesterday",offset:-1},{label:"Today",offset:0},{label:"Tomorrow",offset:1}].map(({label,offset})=>(
          <button key={offset} onClick={()=>setDateOffset(offset)}
            style={{
              flex:1,background:dateOffset===offset?(offset===0?"rgba(239,68,68,.15)":"rgba(139,92,246,.15)"):"rgba(255,255,255,.04)",
              border:`1.5px solid ${dateOffset===offset?(offset===0?"rgba(239,68,68,.4)":"rgba(139,92,246,.4)"):"rgba(255,255,255,.08)"}`,
              borderRadius:10,padding:"7px 4px",
              fontFamily:"var(--font-head)",fontWeight:800,fontSize:12,
              color:dateOffset===offset?(offset===0?"#ef4444":"#C4B5FD"):"rgba(240,240,250,.4)",
      ...
      {loading ? (
        ...
        <div>
          {/* Compact Final Scores bar — always visible when games are done today */}
          {recentEvents.length>0 && dateOffset===0 && (
            <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,padding:"10px 14px",marginBottom:12}}>
              <div style={{fontSize:9,fontWeight:800,color:"var(--muted)",letterSpacing:1.5,marginBottom:8}}>✅ FINAL SCORES</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {recentEvents.map(evt=>(
                  <div key={evt.id} style={{
                    ...
                    <span style={{color:"rgba(240,240,250,.5)",fontSize:10}}>{evt.away?.abbr||evt.away?.name?.slice(0,3).toUpperCase()}</span>
                    <span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:13,color:evt.away?.winner?"#10B981":"rgba(240,240,250,.9)"}}>{evt.away?.score}</span>
                    <span style={{color:"rgba(240,240,250,.3)"}}>-</span>
                    <span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:13,color:evt.home?.winner?"#10B981":"rgba(240,240,250,.9)"}}>{evt.home?.score}</span>
                    <span style={{color:"rgba(240,240,250,.5)",fontSize:10}}>{evt.home?.abbr||evt.home?.name?.slice(0,3).toUpperCase()}</span>
                  </div>
                ))}
              </div>
            </div>
...
function PredictionStatsBar() {
  ...
  if (s.total === 0) return (
    <div style={{background:"linear-gradient(135deg,rgba(139,92,246,.1),rgba(245,158,11,.06))",border:"1px solid rgba(139,92,246,.25)",borderRadius:16,padding:"14px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
      <div style={{fontSize:32}}>🔮</div>
      <div style={{flex:1}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,marginBottom:3}}>Predict the match. Win badges.</div>
        <div style={{fontSize:11,color:"var(--muted)",lineHeight:1.5}}>Tap any upcoming game and pick the winner. Build a streak. Earn legendary status.</div>
      </div>
      <button onClick={sharePredictions} style={{background:"rgba(139,92,246,.15)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,color:"#C4B5FD",padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>📤 Share</button>
    </div>
  );

  return (
    <div style={{background:"rgba(139,92,246,.07)",border:"1px solid rgba(139,92,246,.2)",borderRadius:16,padding:"14px 16px",marginBottom:16}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:13,display:"flex",alignItems:"center",gap:6}}>
          🔮 Predictions
          {milestone && <span style={{fontSize:18}}>{milestone.icon}</span>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {milestone && <div style={{fontSize:10,fontWeight:800,color:milestone.color,background:`${milestone.color}22`,border:`1px solid ${milestone.color}44`,borderRadius:99,padding:"2px 10px"}}>{milestone.label}</div>}
          <button onClick={sharePredictions} style={{background:"rgba(139,92,246,.12)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,color:"#C4B5FD",padding:"3px 10px",fontSize:10,fontWeight:700,cursor:"pointer"}}>📤 Share</button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
        {[
          {label:"Streak",value:`${s.streak}🔥`,color:"#F59E0B"},
          {label:"Best",value:`${s.best}⚡`,color:"#C4B5FD"},
          {label:"Correct",value:`${acc}%`,color:"#10B981"},
          {label:"Points",value:`${s.points}`,color:"#F59E0B"},
        ].map(stat=>(
          <div key={stat.label} style={{background:"rgba(255,255,255,.04)",borderRadius:10,padding:"8px 4px",textAlign:"center"}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:15,color:stat.color}}>{stat.value}</div>
            <div style={{fontSize:9,color:"var(--muted)",marginTop:2,fontWeight:700,letterSpacing:.5}}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      {nextMilestone && (
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
            <div style={{fontSize:10,color:"var(--muted)",fontWeight:600}}>Next: {nextMilestone.icon} {nextMilestone.label}</div>
            <div style={{fontSize:10,color:nextMilestone.color,fontWeight:700}}>{s.streak}/{nextMilestone.n}</div>
          </div>
          <div style={{height:4,background:"rgba(255,255,255,.06)",borderRadius:99,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${progressPct}%`,background:`linear-gradient(90deg,${nextMilestone.color}88,${nextMilestone.color})`,borderRadius:99,transition:"width .5s ease"}}/>
          </div>
        </div>
      )}
      {!nextMilestone && (
        <div style={{textAlign:"center",fontSize:11,color:"#F59E0B",fontWeight:700}}>🐐 GOAT status achieved — legendary</div>
      )}

      {/* All badges earned */}
      {s.streak >= 1 && (
        <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
          {PRED_MILESTONES.filter(m=>m.n<=s.best).map(m=>(
            <div key={m.n} title={m.label} style={{fontSize:18,opacity:s.streak>=m.n?1:.3,filter:s.streak>=m.n?"none":"grayscale(1)",transition:"all .3s"}}>{m.icon}</div>
          ))}
        </div>
      )}
    </div>
...
function PredictionPoll({ evt, user, showToast, onResult }) {
  ...
  useEffect(() => {
    ...
    if (predCorrect) {
      ...
      savePredStats({streak:newStreak,best:newBest,total:old.total+1,correct:old.correct+1,points:old.points+pts,badges:allBadges});
      onResult?.({correct:true, streak:newStreak, points:pts, milestone, gameId:evt.id});
    } else {
      savePredStats({...old, streak:0, total:old.total+1, badges:old.badges||[]});
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={onPrivacy} style={{background:"none",border:"none",color:"rgba(240,240,250,.35)",cursor:"pointer",fontSize:11,fontFamily:"inherit",textDecoration:"underline"}}>Privacy Policy</button>
        <span style={{color:"rgba(255,255,255,.15)",fontSize:11}}>·</span>
        <button onClick={onTerms} style={{background:"none",border:"none",color:"rgba(240,240,250,.35)",cursor:"pointer",fontSize:11,fontFamily:"inherit",textDecoration:"underline"}}>Terms of Service</button>
        <span style={{color:"rgba(255,255,255,.15)",fontSize:11}}>·</span>
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:0}}>
        ...
        <div style={{padding:"0 14px 12px",display:"flex",flexDirection:"column",gap:8}}>
          <button onClick={()=>{setView("sports");setSearch("");setShowAllGames(false);}}
            style={{
              width:"100%",
          ...
          {/* All Games quick link */}
          <button onClick={()=>{setView("sports");setShowAllGames(true);}}
            style={{width:"100%",background:"rgba(139,92,246,.07)",border:"1px solid rgba(139,92,246,.2)",borderRadius:14,padding:"11px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}}
            onTouchStart={e=>e.currentTarget.style.opacity=".8"}
            onTouchEnd={e=>e.currentTarget.style.opacity="1"}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:20}}>🏟️</span>
              <div style={{textAlign:"left"}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:13,color:"#C4B5FD"}}>All Games</div>
                <div style={{fontSize:11,color:"rgba(196,181,253,.55)"}}>Yesterday · Today · Tomorrow</div>
              </div>
            </div>
            <span style={{color:"rgba(139,92,246,.5)",fontSize:16}}>›</span>
          </button>
        </div>
              ...
              {!search.trim() ? (
                ...
                <>
                  ...
                  {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                    : search==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                    : search.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                    : <><LiveSportsSection sportQuery={search} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult} cols={3}/><SportMovieBridge activeSport={search} onSelect={handleSelectMovie}/></>}
                </>
              )}
            </div>
          ) : view==="stats" ? (
            ...
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:18}}>
                  {view==="watchlist" ? "❤️ Saved" : search.trim() ? (searching?"Searching…":`${searchResults.length} results for "${search}"`) : CATEGORY_TABS.find(t=>t.id===view)?.icon+" "+CATEGORY_TABS.find(t=>t.id===view)?.label}
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)"}}>
        ...
        {view==="home"&&!search.trim()&&(
          <div style={{padding:"0 24px 20px",maxWidth:1440,margin:"0 auto"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              ...
              {/* Sports Hub card */}
              <div onClick={()=>{setView("sports");setSearch("");setShowAllGames(false);}}
                style={{
        ...
        <div style={{display:"flex",padding:`${(view==="home"&&!search.trim()||view==="trending"&&!search.trim())?"0":"20px"} 24px 20px`,gap:20,maxWidth:1440,margin:"0 auto"}}>
            ...
            {view==="home"&&!search.trim() ? (
              ...
              <div>
                ...
                <SportCategoryGrid onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                  ...
                  <>
                    <button onClick={()=>{setSearch("");setShowAllGames(false);}} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,.06)",border:"1px solid var(--border)",borderRadius:99,color:"var(--muted)",padding:"5px 12px",fontSize:12,cursor:"pointer",marginBottom:16}}>← Back to Sports Hub</button>
                    {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                      : search==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                      : search.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                      : <LiveSportsSection sportQuery={search} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult} cols={4}/>}
                  </>
                )}
              </div>
            ) : view==="stats" ? (
              ...
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                  <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:18}}>
                    {view==="watchlist" ? "❤️ Saved" : search.trim()
                      ? searching?"Searching…":`${searchResults.length} results for "${search}"`
          ...
          {view==="home"&&!search.trim()&&<aside style={{width:220,flexShrink:0}}>
            {!user&&(
