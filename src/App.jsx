...
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  ...
  if (allFavTeams.length === 0) return (
    <div style={{
      background:"linear-gradient(135deg,rgba(245,158,11,.1),rgba(245,158,11,.05))",
      border:"1.5px solid rgba(245,158,11,.3)",
      borderRadius:16, padding:compact?"10px 10px":"14px 14px",
      display:"flex", flexDirection:"column", gap:compact?6:10, minHeight:compact?90:120,
    }}>
      <div style={{fontSize:compact?20:28,lineHeight:1}}>⭐</div>
      <div>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"var(--gold)",marginBottom:2}}>My Teams</div>
        <div style={{fontSize:compact?9:10,color:"rgba(245,158,11,.6)",lineHeight:1.4}}>{compact?"Tap ⭐ on a sport to follow":"Follow teams to see their next game here."}</div>
      </div>
      {!compact&&<div style={{marginTop:"auto",background:"rgba(245,158,11,.12)",border:"1px solid rgba(245,158,11,.25)",borderRadius:8,padding:"5px 8px",fontSize:9,color:"var(--gold)",fontWeight:700,textAlign:"center"}}>Tap ⭐ on any sport to follow</div>}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"var(--gold)",marginBottom:compact?5:8}}>⭐ My Teams</div>
      {/* Team pills */}
      <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:compact?5:8}}>
        {allFavTeams.map(({sport,name})=>(
          <button key={sport+name}
            onClick={()=>loadTeam(name)}
            style={{
              background:activeTeam===name?"var(--gold)":"rgba(245,158,11,.12)",
              border:`1px solid ${activeTeam===name?"var(--gold)":"rgba(245,158,11,.3)"}`,
              borderRadius:99,
              color:activeTeam===name?"#000":"var(--gold)",
              padding:compact?"2px 8px":"3px 10px",fontSize:compact?9:10,fontWeight:700,cursor:"pointer",
              transition:"all .15s",
            }}>
            {name}
          </button>
      ...
      <div style={{marginTop:"auto"}}>
        {loading ? (
          ...
          <div style={{background:"rgba(0,0,0,.2)",borderRadius:10,padding:"8px 10px"}}>
            <div style={{fontSize:8,color:"rgba(245,158,11,.6)",fontWeight:700,letterSpacing:1,marginBottom:5}}>{result.sport.toUpperCase()}</div>
              ...
              <div style={{flex:1,minWidth:0}}>
                {[result.away,result.home].map((t,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:5,marginBottom:i===0?3:0}}>
                    ...
                    {(result.isLive||result.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:12,color:t.winner?"var(--gold)":"var(--text)",flexShrink:0}}>{t.score}</span>}
                  </div>
                ))}
              </div>
                ...
                {result.isLive
                  ...
                  : result.isOver
                    ...
                    : <div style={{background:"rgba(245,158,11,.1)",border:"1px solid rgba(245,158,11,.2)",borderRadius:6,padding:"3px 6px"}}>
                        <div style={{fontSize:9,fontWeight:700,color:"var(--gold)"}}>{result.localDate}</div>
                        <div style={{fontSize:8,color:"var(--muted)"}}>{result.localTime}</div>
                      </div>
                }
              </div>
            </div>
          ...
          <div style={{fontSize:10,color:"rgba(245,158,11,.5)",textAlign:"center",padding:"8px 0"}}>No upcoming games found</div>
        ) : null}
      </div>
    </div>
  );
...
// ── Banner 2: Find Any Team's Next Game ────────────────────────────────────────
function TeamNextGameSearch({ favoriteTeams }) {
  ...
  return (
    <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(139,92,246,.2)",borderRadius:12,marginBottom:12,overflow:"hidden"}}>
      <div style={{padding:"10px 14px 10px",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:12,color:"#C4B5FD",marginBottom:8}}>🔍 Find Any Team's Next Game</div>
        <div style={{display:"flex",gap:8}}>
          <input
            ...
            style={{flex:1,background:"rgba(255,255,255,.07)",border:"1px solid rgba(139,92,246,.3)",borderRadius:9,color:"var(--text)",padding:"8px 11px",fontSize:12,outline:"none"}}
          />
          <button onClick={()=>searchTeam(query)}
            style={{background:"var(--purple)",border:"none",borderRadius:9,color:"#fff",padding:"8px 14px",fontWeight:800,fontSize:12,cursor:"pointer",flexShrink:0}}>
            {loading?"…":"Go"}
          </button>
        </div>
      </div>
      <TeamGameResult result={result} loading={loading} searched={searched} query={query}/>
    </div>
  );
}
