...
function ProfileModal({ user, profile, tier, watchlist, userRatings, userSubs=[], onClose, onSignOut, onUpgrade, showToast, onEditSubs, onSelectMovie, notifPermission, onRequestNotif, streak }) {
  ...
  // Load watchlist movies — reload whenever watchlist changes
  useEffect(() => {
    if (tab !== "watchlist") return;
    if (watchlist.length === 0) { setWlMovies([]); return; }
    setLoadingWl(true);
    Promise.all(watchlist.slice(0,40).map(async id => {
      try {
        const res = await tmdbFetch(`/movie/${id}?language=en-US`).catch(()=>null);
        if (res?.id) return res;
        return await tmdbFetch(`/tv/${id}?language=en-US`).catch(()=>null);
      } catch { return null; }
    })).then(results => {
        ...
        <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,7,15,.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(245,158,11,.1)",paddingTop:"env(safe-area-inset-top)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",padding:"10px 14px 8px",gap:10}}>
            ...
            <button onClick={()=>{setView("watchlist");setSearch("");}}
              style={{background:view==="watchlist"?"rgba(239,68,68,.15)":"rgba(255,255,255,.06)",border:`1px solid ${view==="watchlist"?"rgba(239,68,68,.4)":"rgba(255,255,255,.1)"}`,borderRadius:10,color:view==="watchlist"?"#ef4444":"var(--muted)",width:36,height:36,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"}}>
              ❤️
            </button>
            {tier==="premium"
