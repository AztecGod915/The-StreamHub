...
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  ...
  const [mode, setMode]               = useState("teams"); // "teams" | "search"
  ...
  const minH = compact ? 90 : 120;

  const GameResult = ({result:r, loading:l, searched:s, query:q}) => {
    if (l) return (
    ...
    if (s && !r) return <div style={{fontSize:9,color:"var(--muted)",textAlign:"center",padding:"6px 0"}}>No games found for "{q}"</div>;
    if (!r) return null;
    return (
      <div style={{background:"rgba(0,0,0,.2)",borderRadius:9,padding:"7px 9px",marginTop:6}}>
        <div style={{fontSize:8,color:"rgba(245,158,11,.6)",fontWeight:700,letterSpacing:1,marginBottom:4}}>{r.sport.toUpperCase()}</div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:6}}>
          <div style={{flex:1,minWidth:0}}>
            {[r.away,r.home].map((t,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:5,marginBottom:i===0?3:0}}>
                {t.logo&&<img src={t.logo} alt="" style={{width:14,height:14,objectFit:"contain",flexShrink:0}}/>}
                <span style={{fontSize:11,fontWeight:700,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</span>
                {(r.isLive||r.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:12,color:t.winner?"var(--gold)":"var(--text)",flexShrink:0}}>{t.score}</span>}
              </div>
            ))}
          </div>
          <div style={{flexShrink:0,textAlign:"center"}}>
            {r.isLive
              ? <div style={{background:"rgba(239,68,68,.2)",border:"1px solid rgba(239,68,68,.4)",borderRadius:6,padding:"3px 6px",color:"#ef4444",fontWeight:800,fontSize:8}}>🔴 LIVE</div>
              : r.isOver
                ? <div style={{fontSize:9,color:"var(--muted)",fontWeight:700}}>FINAL</div>
                : <div style={{background:"rgba(245,158,11,.1)",border:"1px solid rgba(245,158,11,.2)",borderRadius:6,padding:"3px 6px"}}>
                    <div style={{fontSize:9,fontWeight:700,color:"var(--gold)"}}>{r.localDate}</div>
                    <div style={{fontSize:8,color:"var(--muted)"}}>{r.localTime}</div>
                    {r.broadcast&&<div style={{fontSize:7,color:"var(--gold)",fontWeight:700,marginTop:1}}>{r.broadcast}</div>}
                  </div>
            }
          </div>
        </div>
        {r.venue&&<div style={{marginTop:4,fontSize:8,color:"var(--muted)"}}>📍 {r.venue}{r.city?`, ${r.city}`:""}</div>}
      </div>
    );
  };
  ...
  return (
    <div style={{
      ...
      display:"flex", flexDirection:"column", gap:0, minHeight:minH,
    }}>
      {/* Header row: title + mode toggle */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:compact?5:7}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:titleSize,color:"var(--gold)"}}>
          {mode==="teams"?"⭐ My Teams":"🔍 Find Team"}
        </div>
        <div style={{display:"flex",gap:4}}>
          <button onClick={()=>setMode("teams")}
            style={{background:mode==="teams"?"rgba(245,158,11,.25)":"rgba(245,158,11,.08)",border:`1px solid ${mode==="teams"?"rgba(245,158,11,.6)":"rgba(245,158,11,.2)"}`,borderRadius:7,color:"var(--gold)",padding:"2px 7px",fontSize:9,fontWeight:700,cursor:"pointer"}}>
            ⭐
          </button>
          <button onClick={()=>setMode("search")}
            style={{background:mode==="search"?"rgba(245,158,11,.25)":"rgba(245,158,11,.08)",border:`1px solid ${mode==="search"?"rgba(245,158,11,.6)":"rgba(245,158,11,.2)"}`,borderRadius:7,color:"var(--gold)",padding:"2px 7px",fontSize:9,fontWeight:700,cursor:"pointer"}}>
            🔍
          </button>
        </div>
      </div>
      ...
      {mode==="teams" && (<>
        {allFavTeams.length===0
          ? <div style={{fontSize:compact?9:10,color:"rgba(245,158,11,.6)",marginBottom:6}}>Tap ⭐ on any sport to follow teams</div>
          : <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:compact?4:6}}>
              {allFavTeams.map(({sport,name})=>(
                <button key={sport+name} onClick={()=>loadTeam(name)}
                  style={{background:activeTeam===name?"var(--gold)":"rgba(245,158,11,.12)",border:`1px solid ${activeTeam===name?"var(--gold)":"rgba(245,158,11,.3)"}`,borderRadius:99,color:activeTeam===name?"#000":"var(--gold)",padding:compact?"2px 8px":"3px 10px",fontSize:compact?9:10,fontWeight:700,cursor:"pointer",transition:"all .15s"}}>
                  {name}
                </button>
              ))}
            </div>
        ...
        <div style={{marginTop:"auto"}}>
          <GameResult result={result} loading={loading} searched={!!activeTeam} query={activeTeam}/>
        </div>
      </>)}
      ...
      {mode==="search" && (<>
        <div style={{display:"flex",gap:6,marginBottom:4}}>
          <input
            ...
            style={{flex:1,background:"rgba(0,0,0,.2)",border:"1px solid rgba(245,158,11,.3)",borderRadius:8,color:"var(--text)",padding:"6px 9px",fontSize:11,outline:"none"}}
            autoFocus
          />
          <button onClick={()=>searchTeam(query)}
            style={{background:"var(--gold)",border:"none",borderRadius:8,color:"#000",padding:"6px 11px",fontWeight:800,fontSize:11,cursor:"pointer",flexShrink:0}}>
            {searching?"…":"Go"}
          </button>
        </div>
        <div style={{marginTop:"auto"}}>
          <GameResult result={searchResult} loading={searching} searched={searched} query={query}/>
        </div>
      </>)}
    </div>
  );
